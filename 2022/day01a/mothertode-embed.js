  ...
  //====== ./term.js ======
  {
    ...
    Term.default = {
      ...
      throw(source, options = {}) {
        ...
        return `Expected ${this.toString(options)} but found "${source.slice(
          0,
          Term.ERROR_SNIPPET_LENGTH
        )}"`;
      },
    ...
    Term.or = (terms) => ({
      ...
      throw(source, { exceptions = [], ...options } = {}) {
        ...
        const found =
          source.length === 0
            ? "end of input"
            : '"' + source.slice(0, Term.ERROR_SNIPPET_LENGTH) + '"';
        const termNames = longestTerms
