...
const instructions = input.split("").map(v => {
	if (v === "^") return [ 0, 1]
	if (v === "v") return [ 0,-1]
	if (v === "<") return [-1, 0]
	if (v === ">") return [ 1, 0]
...
const SPACE_SIZE = 10
const getCanvasPosition = ([x, y]) => [(x*SPACE_SIZE + canvas.width/2), (y*SPACE_SIZE + canvas.height/1.2)]
...
const draw = () => {
	...
	for (const house of houses) {
		ctx.fillStyle = "white"
		ctx.fillRect(...getCanvasPosition(house), SPACE_SIZE, SPACE_SIZE)
	}
	
	// Santa
	ctx.fillStyle = "red"
	ctx.fillRect(...getCanvasPosition(santaPosition), SPACE_SIZE, SPACE_SIZE)
...
const update = () => {
	...
	santaPosition = [santaPosition[0] + instruction[0], santaPosition[1] + instruction[1]]
	if (!houses.some(house => house[0] === santaPosition[0] && house[1] === santaPosition[1])) {
		houses.push([...santaPosition])
	}
}
